...
class _LedgerSvc extends LedgerService {
  ...
  @override
  Future<LedgerEntry> create(Map<String, dynamic> body) async => LedgerEntry(
    id: 'new',
    employeeId: '',
    employeeName: '',
    date: '',
...
class _DisputeSvc extends DisputeService {
  ...
  Future<List<Dispute>> list({String status = 'open'}) async =>
      status == 'resolved' ? (_r.value ?? []) : (_o.value ?? []);
}
...
AppUser _admin() =>
    AppUser(token: 't', tenantId: 't', employeeId: 'e', role: 'owner');
AppUser _employee() =>
    AppUser(token: 't', tenantId: 't', employeeId: 'e', role: 'employee');
...
DashboardData _dashData() => DashboardData(
  ...
  recentActivity: [
    ActivityItem(
      action: 'attendance',
      description: 'Rahul marked present',
...
Future<void> _screenshot(WidgetTester tester, String name, Widget app) async {
  ...
  await tester.runAsync(() async {
    final boundary =
        find.byType(RepaintBoundary).evaluate().first.findRenderObject()
...
void main() {
  ...
  final ledgerSumF = _Fake<LedgerSummary>()
    ..set(
      LedgerSummary(
        jamaTotal: 80000,
        udhaarTotal: 35000,
  ...
  final disputeOpenF = _Fake<List<Dispute>>()
    ..set([
      Dispute(
        id: 'd1',
        ledgerId: 'l1',
        employeeId: 'e1',
        raisedBy: 'e1',
        reason: 'Incorrect wage',
        status: 'open',
        raisedByName: 'Rahul Kumar',
      ),
    ]);
  final disputeResF = _Fake<List<Dispute>>()
    ..set([
      Dispute(
        id: 'd2',
        ledgerId: 'l2',
  ...
  final holidayF = _Fake<List<Holiday>>()
    ..set([
      Holiday(
        id: 'h1',
  ...
  final advReqF = _Fake<List<AdvanceRequest>>()
    ..set([
      AdvanceRequest(
        id: 'a1',
        employeeId: 'e1',
  ...
  final leavePolF = _Fake<LeavePolicy>()
    ..set(LeavePolicy(paidLeaveDaysPerYear: 15, unpaidLeaveDaysPerYear: 5));
  final dailySumF = _Fake<DailySummaryData>()
    ..set(
      DailySummaryData(
        date: '2026-08-15',
        totalWorkers: 40,
  ...
  final defaulterF = _Fake<List<DefaulterItem>>()
    ..set([
      DefaulterItem(
        name: 'Rahul',
  ...
  final payrollF = _Fake<PayrollResult>()
    ..set(
      PayrollResult(
        totalWage: 80000,
        entries: [
  ...
  final settingsF = _Fake<PayrollSettings>()
    ..set(
      PayrollSettings(
        otThresholdHours: 8,
  ...
  final ovF = _Fake<Map<String, dynamic>>()
    ..set({
      'overview': {
        'profile': {
          'name': 'Rahul Kumar',
  ...
  group('05_ledger_hub', () {
    testWidgets('screen', (tester) async {
      await _screenshot(
        ...
        ProviderScope(
          overrides: [
            ledgerServiceProvider.overrideWithValue(
              _LedgerSvc(ledgerF, ledgerSumF),
            ),
          ],
  ...
  group('06_disputes', () {
    testWidgets('screen', (tester) async {
      await _screenshot(
        ...
        ProviderScope(
          overrides: [
            disputeServiceProvider.overrideWithValue(
              _DisputeSvc(disputeOpenF, disputeResF),
            ),
          ],
  ...
  group('09_advance_requests', () {
    testWidgets('screen', (tester) async {
      await _screenshot(
        ...
        ProviderScope(
          overrides: [
            advanceRequestServiceProvider.overrideWithValue(
              _AdvReqSvc(advReqF),
            ),
          ],
  ...
  group('10_leave_policy', () {
    testWidgets('screen', (tester) async {
      await _screenshot(
        ...
        ProviderScope(
          overrides: [
            leavePolicyServiceProvider.overrideWithValue(
              _LeavePolicySvc(leavePolF),
            ),
          ],
        ...
        ProviderScope(
          overrides: [
            reportServiceProvider.overrideWithValue(
              _ReportSvc(dailySumF, defaulterF),
            ),
          ],
  ...
  group('12_payroll_preview', () {
    testWidgets('screen', (tester) async {
      await _screenshot(
        ...
        ProviderScope(
          overrides: [
            payrollServiceProvider.overrideWithValue(_PayrollSvc(payrollF)),
          ],
