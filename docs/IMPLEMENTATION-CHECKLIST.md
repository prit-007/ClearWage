...
## Phase 10 — v0.8.2 multi-platform CI & Firebase

### Firebase

- [x] `firebase_options.dart` generated by FlutterFire CLI (Android, iOS, macOS, Windows, web).
- [x] Linux falls back to web config (no official Firebase Linux SDK).
...
- [x] `.github/workflows/ci.yml` — 5 platform build jobs (Android, iOS, Windows, macOS, Linux).
- [x] `.github/actions/flutter-prep/action.yml` — Linux deps (ninja-build, libgtk-3-dev).

### Installer

- [x] `app/tool/build_installer.dart` — Windows Inno Setup builder.

### Release

- [x] `CHANGELOG.md` entry for `[0.8.2]`.
